...
class RecordsMixin:
    ...
    async def async_get_alarm_records(self, device_id: str, page_size: int = 10) -> list[dict]:
        """Get the most recent alarm/security records for a device."""
        ...
        params = {
            ...
            "startTime": 0,
            "endTime": 0,
            "targetStandardDpCodes": "alarm_lock",
        }
        resp = await self._request("GET", path, params=params)

        if not resp.get("success"):
            _LOGGER.error("Failed to get alarm records: %s", resp.get("msg"))
